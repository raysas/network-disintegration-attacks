...
base_names <- basename(my_dir_files)
graph_names <- gsub("\\.gml$", "", base_names)
graph_names <- gsub("-", "_", graph_names)
print(graph_names)

############
base_name <- basename('benchmark/arenas-email.gml')
graph_name <- gsub("\\.gml$", "", base_name)
graph_name <- gsub("-", "_", graph_name)
print(graph_name)

g<-read_graph('benchmark/arenas-email.gml', format = 'gml')
N <- vcount(g)
...
write.csv(model_output_test, "data/model_output_test2.csv", row.names = FALSE)

write.csv(model_output_full, "data/model_output_full_test.csv", row.names = FALSE)
